...
├── notes.md                          ← Index of all papers studied
│
...
```
Use `pdm` for package management and dependencies management.
...
**assets/** — Preferred location for exported visual artifacts:
- Prefer Excalidraw for diagrams and systems visuals when the Excalidraw MCP server is available in Codex
- Save the editable source as `*.excalidraw` in `assets/`
- Export a `*.png` or `*.svg` copy into `assets/` for embedding in `notes.md`
- Embed exported images in `notes.md` with standard Markdown when useful, for example:
...
4. Fill notes.md progressively as we go through the learning phases
5. Create implementation files as we reach Phase 5
6. Create visual artifacts as we reach Phase 4
7. At the end, update the root notes.md index

### Diagram Workflow

Use Excalidraw as the default workflow for architecture and systems diagrams when available through Codex MCP.

1. Create or refine the diagram in Excalidraw
2. Save the editable source in `assets/` as `*.excalidraw`
3. Export a static `*.png` or `*.svg` into `assets/`
4. Reference the exported image from `notes.md`

If Excalidraw MCP is unavailable, fall back to a self-contained HTML or SVG artifact in `visuals/`.

### Root notes.md Index Format

The root notes.md maintains an index of all studied papers:
...
Start creating `<paper-slug>/notes.md` with the TL;DR, Problem, and tags.
...
- If using Excalidraw, save the editable source in `assets/` and export a PNG/SVG version for embedding into `notes.md`

Also export a static PNG/SVG to `assets/` for quick reference in the notes.

Embed the exported image in `notes.md` when it materially improves readability.
...
Update the notes.md "What Breaks Without Each Component" table.
...
Update notes.md "Follow-Up Readings" section. Cross-link to other folders
...
1. Ensure `notes.md` is complete with all sections filled
...
5. Update the root `notes.md` index table with the new paper
...
- Generate the structured output in the notes.md format
